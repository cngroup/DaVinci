...
public class RungeKuttaIntegrator implements Integrator {
	 ...
	 *      long)
	 */
	public double integrate(ForceSimulator sim, long timestep) {
		...
		while (iter.hasNext()) {
			...
			item.location[0] = p[0] + (k[0][0] + k[3][0]) / 6.0f
					+ (k[1][0] + k[2][0]) / 3.0f;
			item.location[1] = p[1] + (k[0][1] + k[3][1]) / 6.0f
					+ (k[1][1] + k[2][1]) / 3.0f;
			...
			energy += (item.location[0] - item.plocation[0]) * (item.location[0] - item.plocation[0]) 
					+ (item.location[1] - item.plocation[1]) * (item.location[1] - item.plocation[1]);
		}
		energy /= sim.itemSize();
		return energy;
	}
