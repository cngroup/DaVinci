...
public class ColorLib {
	...
	public static int darker(int c) {
		return rgba(Math.max(0, (int) (scale * red(c))), Math.max(0,
				(int) (scale * green(c))),
	...
	public static int brighter(int c) {
		...
		return rgba(Math.min(255, (int) (r / scale)), Math.min(255,
				(int) (g / scale)), Math.min(255, (int) (b / scale)), alpha(c));
	}
	...
	public static Color saturate(Color c, float sat) {
		float[] hsb = Color.RGBtoHSB(c.getRed(), c.getGreen(), c.getBlue(), null);
		return new Color(ColorLib.hsb(hsb[0], sat, hsb[2]));
	}
	 ...
	 */
	public static final float[] CATEGORY_HUES = { 0f, 1f / 12f, 1f / 6f,
			1f / 3f, 1f / 2f, 7f / 12f, 2f / 3f, /* 3f/4f, */5f / 6f, 11f / 12f };
	 ...
	 */
	public static int[] getCategoryPalette(int size, float s1, float s2,
			float b, float a) {
		int[] cm = new int[size];
		float s = s1;
		for (int i = 0; i < size; i++) {
	...
	public static int[] getGrayscalePalette(int size) {
		...
		for (int i = 0, g; i < size; i++) {
			g = (int) Math
					.round(255 * (0.2f + 0.6f * ((float) i) / (size - 1)));
			cm[size - i - 1] = gray(g);
		}
		return cm;
	}
...
//		paletteInts = new int[] { 
//				new Color(142, 180, 227).getRGB(),
//				new Color(153, 204, 0).getRGB(),
//				new Color(255, 255, 153).getRGB(),
//				new Color(255, 102, 0).getRGB(),
//				new Color(183, 222, 232).getRGB(),
//				new Color(204, 193, 218).getRGB()};
		
		// prefuse palette
		// paletteInts = ColorLib.getCategoryPalette(10);
		palette = new Color[paletteInts.length];
		for (int i = 0; i < paletteInts.length; i++) {
			palette[(i + 0) % paletteInts.length] = new Color(paletteInts[i]);
		}
	...
	public static Map<String, Integer> m_colormap = new HashMap<String, Integer>();
	static {
		String[] categories = new String[]{"cancer", "diabetes", "kidney disorder", "heart disease", "hiv", "high blood pressure"};
		for(int i = 0; i < categories.length; ++i) {
			m_colormap.put(categories[i], i);
		}
	}
	public static Color getColor(String value) {
		Integer idx = m_colormap.get(value);
		if (idx == null) {
			idx = id;
			m_colormap.put(value, id);
			id++;
		}
